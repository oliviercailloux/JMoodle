...
public record SaveGrade (int userid, double grade, int attemptnumber, boolean addattempt,
    String workflowstate, Optional<GradePluginData> plugindata,
    Optional<AdvancedGradingData> advancedgradingdata) {
   ...
   *        3 and 7, say), though non consecutive attempts are probably begging for trouble as it
   *        does not seem that the gui is designed for this use case.
   * @return the send grade object
   */
  public static SaveGrade overwriteOrAdd(int userid, double grade, int attemptnumber) {
    return new SaveGrade(userid, grade, attemptnumber, true, "graded", Optional.empty(),
        Optional.empty());
  }
  ...
  public SaveGrade withFeedback(Feedback feedback) {
    return new SaveGrade(userid, grade, attemptnumber, addattempt, workflowstate,
        Optional.of(new GradePluginData(Optional.of(feedback), Optional.empty())),
        Optional.empty());
  }
}
